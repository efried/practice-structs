...
public class TreeMap<K,V> extends AbstractSortedMap<K,V> {
    ...
    protected static class BalanceableBinaryTree<K, V>
        ...
        static class BSTNode<E> extends Node<E> {
            int aux = 0;
    ...
    private BalanceableBinaryTree<K, V> tree = new BalanceableBinaryTree<>();
    ...
    Position<Entry<K, V>> parent(Position<Entry<K, V>> p) {
        return tree.parent(p);
    }

    Position<Entry<K, V>> left(Position<Entry<K, V>> p) {
        return tree.left(p);
    }
    ...
    private Position<Entry<K, V>> sibling(Position<Entry<K, V>> p) {
        return tree.sibling(p);
    }

    boolean isRoot(Position<Entry<K, V>> p) {
    ...
    private void set(Position<Entry<K, V>> p, Entry<K, V> e) {
        tree.set(p, e);
    }

    private Entry<K, V> remove(Position<Entry<K, V>> p) {
        return tree.remove(p);
    }

    void rotate(Position<Entry<K, V>> p) {
        tree.rotate(p);
    }
